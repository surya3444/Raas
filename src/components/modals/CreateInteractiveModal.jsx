...
const CreateInteractiveModal = ({ user, currentLayoutCount, currentTotalPlots, onClose }) => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div className="glass-panel w-full max-w-lg p-6 bg-[#121214] border border-white/10 shadow-2xl max-h-[90vh] overflow-y-auto custom-scrollbar">
                ...
                <div className="flex justify-between items-center mb-6">
                    <div>
                        <h2 className="text-lg font-bold text-white">Create Interactive Grid</h2>
                        <p className="text-[10px] text-gray-500 mt-1">
                            Capacity: <span className="text-white">{currentTotalPlots}</span> / <span className="text-blue-400">{globalMaxPlots}</span> used.
                            <br/>
                            Available: <span className="text-green-400 font-bold">{remainingPlots}</span>
                        </p>
                    </div>
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        ...
                        <input className="form-input w-full bg-black/40 border border-white/10 rounded p-2.5 text-sm text-white focus:border-blue-500 outline-none" 
                            value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} 
                            placeholder="e.g. Sunrise Avenue"
                        />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="text-[10px] text-gray-500 uppercase font-bold block mb-1">Plot Prefix</label>
                            <input className="form-input w-full bg-black/40 border border-white/10 rounded p-2.5 text-sm text-white focus:border-blue-500 outline-none" 
                        ...
                        <div>
                            ...
                            <input type="number" className="form-input w-full bg-black/40 border border-white/10 rounded p-2.5 text-sm text-white focus:border-blue-500 outline-none" 
                                value={formData.count} onChange={e => setFormData({...formData, count: e.target.value})} 
                                max={remainingPlots}
                            />
                            {Number(formData.count) > remainingPlots && (
                                <p className="text-[10px] text-red-500 mt-1">Exceeds limit ({remainingPlots})</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea className="w-full bg-black/40 border border-white/10 rounded p-2.5 text-sm text-white focus:border-blue-500 outline-none min-h-[60px]"
                    ...
                    <div>
                        ...
                        <input className="w-full bg-black/40 border border-white/10 rounded p-2.5 text-sm text-blue-400 focus:border-blue-500 outline-none"
                    ...
                    <div className="flex gap-2 pt-4 border-t border-white/10">
                        ...
                        <button 
                            ...
                            className="flex-1 bg-brand hover:bg-blue-600 text-white text-xs font-bold py-3 rounded transition flex justify-center items-center gap-2 disabled:opacity-50"
                        >
                            {isLoading ? <Loader2 className="animate-spin" size={16} /> : "Create Grid"}
                        </button>
                    </div>
                </div>
            </div>
...
export default CreateInteractiveModal;